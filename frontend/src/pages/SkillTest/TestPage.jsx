...
const TestPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    //call api
    result+="Check the answer with the provided questions. If at least 60% is answered correctly then user will Pass ( in response give boolean true), else Fail (in response give boolean false). Check whether the provided solution is AI generated or not. If AI generated then give a partial number. You just give the response as true or false. Don't add any punctuation, space, blank line or something else, just a word."
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        if(text=="true \n" || text=="true" ){
          ...
          axiosInstance
          .post(`/api/skill/certificate?userEmail=${user.email}&type=${topic}&secretCode=${import.meta.env.secretCode}`)
          .then((res) => {
  ...
  const renderQuestion = () => {
    ...
    return (
      ...
      <div className="flex flex-col items-center">
        <h2 className="text-2xl font-bold">{question.type}</h2> {/* Question Type */}
        <p className="text-xl mt-4">{question.question}</p> {/* Question Text */}
        <textarea
  ...
  if (!isUserLoading && !user) {
    return (
      <div className="flex flex-col items-center p-6">
        <h1 className="text-3xl font-bold mb-6">Please log in to take the test</h1>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      {!isTestStarted ? (
        <>
          <p className="text-lg mb-4">Note: You will have 30 minutes to complete this exam.</p>
          <button
        ...
        <div className="w-full max-w-4xl">
          <div className="flex justify-between mb-4">
            <p className="text-lg">Time Left: {Math.floor(timeLeft / 60)}:{timeLeft % 60}</p>
          </div>
          {renderQuestion()}
        </div>
      )}

      {loading && <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-black p-6 rounded shadow-lg">Your result is being generated...</div>
      </div>}
    </div>
  );
};
